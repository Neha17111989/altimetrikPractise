...
public class SortArrayElementsByFrequencyProgram {
	private static void sortArrayElementsByFrequency(int[] inputArray) {
		...
		/*
		 * elementCountMap.entrySet().stream().sorted(Collections.reverseOrder(Map.Entry
		 * .comparingByValue())) .forEach(entry -> { for (int i = 1; i <=
		 * entry.getValue(); i++) sortedElements.add(entry.getKey()); });
		 */
...
//Printing sorted array elements in descending order of their frequency
		 ...
		 * 
		 * ArrayList<Entry<Integer, Integer>> listOfEntry = new
		 * ArrayList<>(elementCountMap.entrySet());
		 * 
		 * //Sort listOfEntry based on values //Collections.sort(listOfEntry);
		 * 
		 ...
		 * 
		 * System.out.
		 * println("Sorted Array Elements In Descending Order Of Their Frequency :");
		 * 
		 * System.out.print("[ ");
		 * 
		 * for (Entry<Integer, Integer> entry : listOfEntry) { int frequency =
		 * entry.getValue(); System.out.println(frequency); while (frequency >= 1) {
